defmodule Karma.S3 do
  ...
  def download_many(urls) do
    ...
    |> Enum.to_list()
    |> Enum.filter(fn {_async_res, {res, _id, _path}} -> res != :error end)
    |> Enum.map(fn {_async_res, { _res, id, path}} -> {id, path} end)
  end
  ...
  def download_with_id({id, url}) do
    ...
    file_destination =
      System.cwd() <> "/tmp/altered_document_" <> id_string <> ".pdf"

    download(url, file_destination)
    |> Tuple.insert_at(1, id)
  end
